...
def report_violation(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            violation, challan = create_violation_and_challan(
            ...
            messages.success(
                request,
                f'Challan #{challan.pk} created for {form.cleaned_data["vehicle"].plate_number} '
                f'(Rs. {challan.fine_amount}). Email sent to {challan.violation.vehicle.owner.email}.'
            )
...
def all_challans(request):
    queryset = Challan.objects.select_related(
        'violation', 'violation__vehicle', 'violation__vehicle__owner', 'issued_by'
    ).order_by('-issued_at')
    ...
    pending_count = queryset.filter(status=Challan.STATUS_PENDING).count()

    return render(request, 'challan/all_challan.html', {
        'challans': queryset,
        'search': search,
        'status': status,
...
def my_challans(request):
    ...
    ).filter(issued_by=request.user).order_by('-issued_at')
    ...
    return render(request, 'challan/my_challans.html', {
        'challans': queryset,
        'search': search,
        'status': status,
        'total_count': total_count,
